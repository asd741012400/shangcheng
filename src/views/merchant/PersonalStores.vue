<template>
    <div class="PersonalStores">
        <div class="logo"><img :src="storeInfo.wechat_img"></div>
        <div class="shop_name">
            <p>
                <input type="text" v-if="compile">
                <span v-else>{{storeInfo.shop_info || storeInfo.username+'的店'}}</span>
                <i @click="compileFn"><img src="../../assets/merchant/icon_compile.png" alt=""></i>
            </p>
            <a>ID：{{storeInfo.user_id}}</a>
        </div>
...
export default {
    ...
    methods: {
        ...
        async getInfo() {
            let res = await this.$getRequest('/store/MyStore', { user_id: this.user.user_id, page: this.page })
            this.storeInfo = res.data.data.user_info
            this.list = res.data.data.card_list
            this.currSize = res.data.data.card_list.length
            this.pageSize = res.data.data.count
        },
        async getInfoMore() {
            let res = await this.$getRequest('/store/MyStore', { user_id: this.user.user_id, page: this.page })
            this.storeInfo = res.data.data.user_info
            this.list = this.list.concat(res.data.data.card_list);
            this.currSize = res.data.data.card_list.length
            this.pageSize = res.data.data.count
        },
